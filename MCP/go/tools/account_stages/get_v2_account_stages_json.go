...
func Get_v2_account_stages_jsonHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v2/account_stages.json%s", cfg.BaseURL, queryString)
...
func CreateGet_v2_account_stages_jsonTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_account_stages_json",
		mcp.WithDescription("List account stages"),
		mcp.WithArray("ids", mcp.Description("IDs of account stages to fetch. If a record can't be found, that record won't be returned and your request will be successful")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v2_account_stages_jsonHandler(cfg),
	}
}
