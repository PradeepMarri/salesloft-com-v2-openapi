...
func Get_v2_call_data_records_jsonHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["has_call"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("has_call=%v", val))
		}
		...
		url := fmt.Sprintf("%s/v2/call_data_records.json%s", cfg.BaseURL, queryString)
...
func CreateGet_v2_call_data_records_jsonTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_call_data_records_json",
		mcp.WithDescription("List call data records"),
		mcp.WithArray("ids", mcp.Description("IDs of call data records to fetch. If a record can't be found, that record won't be returned and your request will be successful")),
		mcp.WithBoolean("has_call", mcp.Description("Return only call data records which have or do not have a call logged for them")),
		...
		mcp.WithArray("person_id", mcp.Description("Filters list by person_id. Multiple person ids can be applied")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v2_call_data_records_jsonHandler(cfg),
	}
}
