...
func Get_v2_crm_activity_fields_jsonHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v2/crm_activity_fields.json%s", cfg.BaseURL, queryString)
...
func CreateGet_v2_crm_activity_fields_jsonTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_crm_activity_fields_json",
		mcp.WithDescription("List crm activity fields"),
		mcp.WithString("source", mcp.Description("Return only records with this source")),
		mcp.WithString("sort_by", mcp.Description("Key to sort on, must be one of: title, updated_at. Defaults to title")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v2_crm_activity_fields_jsonHandler(cfg),
	}
}
