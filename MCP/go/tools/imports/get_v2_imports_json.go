...
func Get_v2_imports_jsonHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["user_ids"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("user_ids=%v", val))
		}
		...
		url := fmt.Sprintf("%s/v2/imports.json%s", cfg.BaseURL, queryString)
...
func CreateGet_v2_imports_jsonTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_imports_json",
		mcp.WithDescription("List imports"),
		mcp.WithArray("ids", mcp.Description("IDs of imports to fetch. If a record can't be found, that record won't be returned and your request will be successful")),
		mcp.WithArray("user_ids", mcp.Description("ID of users to fetch imports for. Using this filter will return an empty array for non-admin users who request other user's imports")),
		mcp.WithString("sort_by", mcp.Description("Key to sort on, must be one of: created_at, updated_at. Defaults to created_at")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v2_imports_jsonHandler(cfg),
	}
}
