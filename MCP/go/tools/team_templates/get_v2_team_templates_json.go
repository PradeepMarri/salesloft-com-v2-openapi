...
func Get_v2_team_templates_jsonHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v2/team_templates.json%s", cfg.BaseURL, queryString)
...
func CreateGet_v2_team_templates_jsonTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_team_templates_json",
		mcp.WithDescription("List team templates"),
		mcp.WithArray("ids", mcp.Description("IDs of team templates to fetch. If a record can't be found, that record won't be returned and your request will be successful")),
		...
		mcp.WithArray("tag", mcp.Description("Filters team templates by tags applied to the template, not to exceed 100 tags")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v2_team_templates_jsonHandler(cfg),
	}
}
