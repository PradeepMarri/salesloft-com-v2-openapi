...
func Get_v2_cadences_jsonHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["team_cadence"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("team_cadence=%v", val))
		}
		if val, ok := args["shared"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("shared=%v", val))
		}
		if val, ok := args["owned_by_guid"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("owned_by_guid=%v", val))
		}
		if val, ok := args["people_addable"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("people_addable=%v", val))
		}
		if val, ok := args["name"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("name=%v", val))
		}
		if val, ok := args["group_ids"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("group_ids=%v", val))
		}
		...
		url := fmt.Sprintf("%s/v2/cadences.json%s", cfg.BaseURL, queryString)
...
func CreateGet_v2_cadences_jsonTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_cadences_json",
		mcp.WithDescription("List cadences"),
		mcp.WithArray("ids", mcp.Description("IDs of cadences to fetch. If a record can't be found, that record won't be returned and your request will be successful")),
		...
		mcp.WithBoolean("team_cadence", mcp.Description("Filters cadences by whether they are a team cadence or not")),
		mcp.WithBoolean("shared", mcp.Description("Filters cadences by whether they are shared")),
		mcp.WithArray("owned_by_guid", mcp.Description("Filters cadences by the owner's guid. Multiple owner guids can be applied")),
		mcp.WithBoolean("people_addable", mcp.Description("Filters cadences by whether they are able to have people added to them")),
		mcp.WithArray("name", mcp.Description("Filters cadences by name")),
		mcp.WithString("group_ids", mcp.Description("Filters by group ids. Also supports group ids passed in as a JSON array string")),
		mcp.WithBoolean("archived", mcp.Description("Filters by whether the Cadences have been archived. Excluding this field will result in both archived and unarchived Cadences to return.")),
		mcp.WithString("sort_by", mcp.Description("Key to sort on, must be one of: created_at, updated_at, name. Defaults to updated_at")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v2_cadences_jsonHandler(cfg),
	}
}
