...
func Get_v2_call_sentiments_jsonHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v2/call_sentiments.json%s", cfg.BaseURL, queryString)
...
func CreateGet_v2_call_sentiments_jsonTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_call_sentiments_json",
		mcp.WithDescription("List call sentiments"),
		mcp.WithString("name", mcp.Description("Filters call sentiments by name")),
		mcp.WithString("sort_by", mcp.Description("Key to sort on, must be one of: name, updated_at. Defaults to name")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v2_call_sentiments_jsonHandler(cfg),
	}
}
