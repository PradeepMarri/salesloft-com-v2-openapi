...
func Get_v2_tags_jsonHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v2/tags.json%s", cfg.BaseURL, queryString)
...
func CreateGet_v2_tags_jsonTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_tags_json",
		mcp.WithDescription("List team tags"),
		mcp.WithString("search", mcp.Description("Filters tags by name")),
		mcp.WithArray("ids", mcp.Description("Filters tags by their IDs")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v2_tags_jsonHandler(cfg),
	}
}
