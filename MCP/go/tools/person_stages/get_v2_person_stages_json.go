...
func Get_v2_person_stages_jsonHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v2/person_stages.json%s", cfg.BaseURL, queryString)
...
func CreateGet_v2_person_stages_jsonTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_person_stages_json",
		mcp.WithDescription("List person stages"),
		mcp.WithArray("ids", mcp.Description("IDs of person stages to fetch.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v2_person_stages_jsonHandler(cfg),
	}
}
