...
func Get_v2_activities_calls_jsonHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["sentiment"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("sentiment=%v", val))
		}
		if val, ok := args["disposition"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("disposition=%v", val))
		}
		...
		url := fmt.Sprintf("%s/v2/activities/calls.json%s", cfg.BaseURL, queryString)
...
func CreateGet_v2_activities_calls_jsonTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_activities_calls_json",
		mcp.WithDescription("List calls"),
		mcp.WithArray("ids", mcp.Description("IDs of calls to fetch. If a record can't be found, that record won't be returned and your request will be successful")),
		...
		mcp.WithArray("user_guid", mcp.Description("Filters list to only include guids")),
		mcp.WithArray("person_id", mcp.Description("Filters calls by person_id. Multiple person ids can be applied")),
		mcp.WithArray("sentiment", mcp.Description("Filters calls by sentiment. Sentiment matches are exact and case sensitive. Multiple sentiments are allowed.")),
		mcp.WithArray("disposition", mcp.Description("Filters calls by disposition. Disposition matches are exact and case sensitive. Multiple dispositions are allowed.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v2_activities_calls_jsonHandler(cfg),
	}
}
