...
func Get_v2_successes_jsonHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v2/successes.json%s", cfg.BaseURL, queryString)
...
func CreateGet_v2_successes_jsonTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_successes_json",
		mcp.WithDescription("List successes"),
		mcp.WithArray("ids", mcp.Description("IDs of successes to fetch. If a record can't be found, that record won't be returned and your request will be successful")),
		mcp.WithArray("person_id", mcp.Description("Filters successes by person_id. Multiple person ids can be applied")),
		...
		mcp.WithString("sort_by", mcp.Description("Key to sort on, must be one of: created_at, updated_at, succeeded_at. Defaults to updated_at")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v2_successes_jsonHandler(cfg),
	}
}
