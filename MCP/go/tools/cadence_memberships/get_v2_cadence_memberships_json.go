...
func Get_v2_cadence_memberships_jsonHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["currently_on_cadence"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("currently_on_cadence=%v", val))
		}
...
func CreateGet_v2_cadence_memberships_jsonTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_cadence_memberships_json",
		mcp.WithDescription("List cadence memberships"),
		mcp.WithArray("ids", mcp.Description("IDs of cadence memberships to fetch. If a record can't be found, that record won't be returned and your request will be successful")),
		mcp.WithNumber("person_id", mcp.Description("ID of the person to find cadence memberships for")),
		mcp.WithNumber("cadence_id", mcp.Description("ID of the cadence to find cadence memberships for")),
		...
		mcp.WithBoolean("currently_on_cadence", mcp.Description("If true, return only cadence memberships for people currently on cadences.  If false, return cadence memberships for people who have been removed from or have completed a cadence.")),
		mcp.WithString("sort_by", mcp.Description("Key to sort on, must be one of: added_at, updated_at. Defaults to updated_at")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v2_cadence_memberships_jsonHandler(cfg),
	}
}
