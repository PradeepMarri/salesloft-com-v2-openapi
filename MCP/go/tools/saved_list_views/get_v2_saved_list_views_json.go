...
func Get_v2_saved_list_views_jsonHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v2/saved_list_views.json%s", cfg.BaseURL, queryString)
...
func CreateGet_v2_saved_list_views_jsonTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_saved_list_views_json",
		mcp.WithDescription("List saved list views"),
		mcp.WithArray("ids", mcp.Description("IDs of saved list views to fetch. If a record can't be found, that record won't be returned and your request will be successful")),
		mcp.WithString("view", mcp.Description("Type of saved list views to fetch.")),
		mcp.WithString("sort_by", mcp.Description("Key to sort on, must be one of: name. Defaults to name")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v2_saved_list_views_jsonHandler(cfg),
	}
}
