...
func Get_v2_custom_fields_jsonHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["field_type"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("field_type=%v", val))
		}
		...
		url := fmt.Sprintf("%s/v2/custom_fields.json%s", cfg.BaseURL, queryString)
...
func CreateGet_v2_custom_fields_jsonTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_custom_fields_json",
		mcp.WithDescription("List custom fields"),
		mcp.WithArray("ids", mcp.Description("IDs of custom fields to fetch.")),
		mcp.WithString("field_type", mcp.Description("Type of field to fetch. Value must be one of: person, company, opportunity")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v2_custom_fields_jsonHandler(cfg),
	}
}
