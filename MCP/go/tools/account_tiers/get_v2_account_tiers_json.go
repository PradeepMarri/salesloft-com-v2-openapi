...
func Get_v2_account_tiers_jsonHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v2/account_tiers.json%s", cfg.BaseURL, queryString)
...
func CreateGet_v2_account_tiers_jsonTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_account_tiers_json",
		mcp.WithDescription("List Account Tiers"),
		mcp.WithArray("ids", mcp.Description("IDs of Account Tiers to fetch. If a record can't be found, that record won't be returned and your request will be successful")),
		mcp.WithArray("name", mcp.Description("Filters Account Tiers by name. Multiple names can be applied")),
		mcp.WithString("sort_by", mcp.Description("Key to sort on, must be one of: created_at, updated_at, order. Defaults to updated_at")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v2_account_tiers_jsonHandler(cfg),
	}
}
