...
func Get_v2_pending_emails_jsonHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v2/pending_emails.json%s", cfg.BaseURL, queryString)
...
func CreateGet_v2_pending_emails_jsonTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_pending_emails_json",
		mcp.WithDescription("Fetches a list of emails ready to be sent by an external email service. Only emails sent with an External Email Client will appear here."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v2_pending_emails_jsonHandler(cfg),
	}
}
