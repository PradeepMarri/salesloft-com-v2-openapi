...
func Get_v2_phone_number_assignments_jsonHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v2/phone_number_assignments.json%s", cfg.BaseURL, queryString)
		...
		var result []PhoneNumberAssignment
...
func CreateGet_v2_phone_number_assignments_jsonTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_phone_number_assignments_json",
		mcp.WithDescription("List phone number assignments"),
		mcp.WithArray("ids", mcp.Description("IDs of phone number assignments to fetch")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v2_phone_number_assignments_jsonHandler(cfg),
	}
}
