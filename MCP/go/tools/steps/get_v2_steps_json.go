...
func Get_v2_steps_jsonHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["has_due_actions"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("has_due_actions=%v", val))
		}
		...
		url := fmt.Sprintf("%s/v2/steps.json%s", cfg.BaseURL, queryString)
...
func CreateGet_v2_steps_jsonTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_steps_json",
		mcp.WithDescription("List steps"),
		mcp.WithArray("ids", mcp.Description("IDs of steps to fetch.")),
		mcp.WithNumber("cadence_id", mcp.Description("Filter by cadence ID")),
		mcp.WithString("type", mcp.Description("Filter by step type")),
		mcp.WithBoolean("has_due_actions", mcp.Description("Filter by whether a step has due actions")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v2_steps_jsonHandler(cfg),
	}
}
