...
func Get_v2_custom_roles_jsonHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v2/custom_roles.json%s", cfg.BaseURL, queryString)
...
func CreateGet_v2_custom_roles_jsonTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_custom_roles_json",
		mcp.WithDescription("List custom roles"),
		mcp.WithArray("ids", mcp.Description("IDs of roles to fetch.")),
		mcp.WithString("sort_by", mcp.Description("Key to sort on, must be one of: id, name. Defaults to id")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v2_custom_roles_jsonHandler(cfg),
	}
}
