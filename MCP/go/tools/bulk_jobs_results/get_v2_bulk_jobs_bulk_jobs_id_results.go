...
func Get_v2_bulk_jobs_bulk_jobs_id_resultsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v2/bulk_jobs/%s/results%s", cfg.BaseURL, bulk_jobs_id, queryString)
...
func CreateGet_v2_bulk_jobs_bulk_jobs_id_resultsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_bulk_jobs_bulk_jobs_id_results",
		mcp.WithDescription("List job data for a completed bulk job."),
		mcp.WithNumber("bulk_jobs_id", mcp.Required(), mcp.Description("The id for the Bulk Job")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v2_bulk_jobs_bulk_jobs_id_resultsHandler(cfg),
	}
}
