...
func Get_v2_bulk_jobsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v2/bulk_jobs%s", cfg.BaseURL, queryString)
...
func CreateGet_v2_bulk_jobsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_bulk_jobs",
		mcp.WithDescription("List bulk jobs"),
		mcp.WithArray("state", mcp.Description("The state of the bulk job. Accepts multiple states. Each state must be one of: open, executing, done")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v2_bulk_jobsHandler(cfg),
	}
}
