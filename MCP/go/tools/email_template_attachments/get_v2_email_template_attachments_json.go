...
func Get_v2_email_template_attachments_jsonHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["email_template_id"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("email_template_id=%v", val))
		}
		...
		url := fmt.Sprintf("%s/v2/email_template_attachments.json%s", cfg.BaseURL, queryString)
		...
		var result []EmailTemplateAttachment
...
func CreateGet_v2_email_template_attachments_jsonTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_email_template_attachments_json",
		mcp.WithDescription("List email template attachments"),
		mcp.WithArray("ids", mcp.Description("IDs of email template attachments to fetch. If a record can't be found, that record won't be returned and your request will be successful")),
		mcp.WithArray("email_template_id", mcp.Description("Filters email template attachments by email template IDs")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v2_email_template_attachments_jsonHandler(cfg),
	}
}
