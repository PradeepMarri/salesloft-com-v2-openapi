...
func Get_v2_notes_jsonHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["associated_with_type"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("associated_with_type=%v", val))
		}
		if val, ok := args["associated_with_id"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("associated_with_id=%v", val))
		}
		...
		url := fmt.Sprintf("%s/v2/notes.json%s", cfg.BaseURL, queryString)
...
func CreateGet_v2_notes_jsonTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_notes_json",
		mcp.WithDescription("List notes"),
		mcp.WithString("associated_with_type", mcp.Description("Case insensitive type of item with which the note is associated.  Value must be one of: person, account")),
		mcp.WithNumber("associated_with_id", mcp.Description("ID of the item with which the note is associated.  The associated_with_type must also be present if this parameter is used")),
		...
		mcp.WithArray("ids", mcp.Description("IDs of notes to fetch. If a record can't be found, that record won't be returned and your request will be successful")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v2_notes_jsonHandler(cfg),
	}
}
